...
class BasePage():
    ...
    def login_base(self, user_number, password):
        ...
        log.info("开始执行登录操作")
        log.info("----" * 15)
        inputting(driver=self.driver, type='location_type', section_name='登录页面', name='输入框', txt=user_number)
        log.info("输入账号")
        inputting(driver=self.driver, type='location_type', section_name='登录页面', name='密码', txt=password)
        log.info("输入密码")
        clicking(driver=self.driver, type='location_type', section_name='登录页面', name='登录按钮')
        log.info("点击登录")
